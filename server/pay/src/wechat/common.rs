...
use openssl::{base64::decode_block, hash::MessageDigest, sign::Verifier, x509::X509};
use reqwest::Url;
use std::{fs, path};
...
pub trait BaseTrait {
    ...
    /// ```
    fn notify<'a, U: DeserializeOwned>(
        &'a self,
        nonce_str: &'a str,
        timestamp: &'a str,
        body: &'a str,
    ...
    /// 根据商家订单号查询订单
    fn query_order(&self, out_trade_no: &str) -> BoxFuture<'_, ResourceOrderBody>;
    /// 根据微信支付订单号查询订单
    fn query_order_by_transaction_id(
        &self,
        transaction_id: &str,
    ) -> BoxFuture<'_, ResourceOrderBody>;
    /// 关闭订单
    fn close_order(&self, out_trade_no: &str) -> BoxFuture<'_, ()>;
    /// 下载证书
    fn download_cert(&self) -> BoxFuture<'_, Vec<String>>;
    /// 构建请求client 同时设置好请求头
...
impl BaseTrait for Payment<WechatConfig> {
    fn create_order(
        ...
        let fut = async move {
            ...
            let rs = self
                .do_request::<CreateOrderResponse>(&url, "POST", &order_body)
                .await?;
            ...
            match trade_type {
                TradeType::JSAPI => {
                    let pay_sign = generate_signature(
                        vec![&app_id, &time_stamp, &nonce_str, &package],
                        &self.config.apiclient_key,
                    )
                    .unwrap();
                ...
                TradeType::App => {
                    let pay_sign = generate_signature(
                        vec![&app_id, &time_stamp, &nonce_str, &prepay_id],
                        &self.config.apiclient_key,
                    )
                    .unwrap();
    ...
    fn notify<'a, U: DeserializeOwned>(
        ...
        let fut = async move {
            let is_valid = self
                .verify_signature(vec![timestamp, nonce_str, body], signature, serial)
                .await?;
            if !is_valid {
            ...
            let associated_data = notify_content
                .resource
                .associated_data
                .unwrap_or("".to_string());
    ...
    fn query_order(&self, out_trade_no: &str) -> BoxFuture<'_, ResourceOrderBody> {
        ...
        let fut = async move { self.do_request::<ResourceOrderBody>(&url, "GET", &"").await };
        Box::pin(fut)
    }
    fn query_order_by_transaction_id(
        &self,
    ...
    fn build_request_builder(
        ...
        let base_url =
            Url::parse("https://api.mch.weixin.qq.com/").map_err(|_e| e("parse url error"))?;
...
mod tests {
    ...
    async fn test_query_order() {
        ...
        let result = payment
            .query_order_by_transaction_id("4200002321202407011113597346")
            .await;
        if result.is_err() {
            let error = result.err().unwrap();
            println!("{}", error);
    ...
    async fn test_notify() {
        ...
        //println!("{}",_body);
        let result = payment
            .notify::<ResourceOrderBody>(nonce_str, timestamp, &body, signature, serial)
            .await;
        if result.is_err() {
            let error = result.err().unwrap();
            println!("{}", error);
    ...
    async fn test_download_cert() {
        ...
        if result.is_err() {
            ...
            let file = result
                .unwrap()
                .into_iter()
                .filter(|x| x.contains("5AD141C1086A7945A1394A8AEAA9EB0619751859"))
                .next();
            println!("{:?}", file);

use casbin::{prelude::*, CoreApi, MgmtApi};
...
impl CasbinService {
    pub async fn new(database_url: &str) -> Result<Self, AppError> {
        ...
        let adapter = SqlxAdapter::new(database_url, 8).await
            .map_err(|e| AppError::internal_error(format!("Failed to create Casbin adapter: {}", e)))?;

        // 创建Enforcer
        let enforcer = Enforcer::new("casbin_model.conf", adapter).await
            .map_err(|e| AppError::internal_error(format!("Failed to create Casbin enforcer: {}", e)))?;
    ...
    pub async fn add_policy(&self, sub: &str, obj: &str, act: &str) -> Result<bool, AppError> {
        ...
        e.add_policy(vec![sub.to_string(), obj.to_string(), act.to_string()]).await
    ...
    pub async fn remove_policy(&self, sub: &str, obj: &str, act: &str) -> Result<bool, AppError> {
        let mut e = self.enforcer.write().await;
        e.remove_policy(vec![sub.to_string(), obj.to_string(), act.to_string()]).await
    ...
    pub async fn add_role_for_user(&self, user: &str, role: &str) -> Result<bool, AppError> {
        let mut e = self.enforcer.write().await;
        e.add_grouping_policy(vec![user.to_string(), role.to_string()]).await
    ...
    pub async fn delete_role_for_user(&self, user: &str, role: &str) -> Result<bool, AppError> {
        let mut e = self.enforcer.write().await;
        e.remove_grouping_policy(vec![user.to_string(), role.to_string()]).await
